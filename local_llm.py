...
async def get_character_data(user_id, character):
   ...
   with open(f'characters/{character}.json') as read_file:
        ...
        history = await get_cache_history(user_id)
...
async def get_prompt(user_id, user_name, user_message, regenerate, character_data):
...
async def update_user_data(user_id: str, response: dict):
...
async def generate_response(user_id, user_name, user_message):
    character_data = await get_character_data(user_id, "main_joe-rogan_json")
    # handling embeding and updating them
    # checking if user message history is less than 15
    handler = ChromaHandler(path="./chroma")
    num_of_messages = handler.read_user_file(str(user_id))
    if not len(num_of_messages) < 15:
        response_pairs = handler.get_or_query_collection(str(user_id), user_message, n_results=3)
        print("response_pairs: ", response_pairs)
        await update_user_data(str(user_id), response_pairs)
    prompt = await get_prompt(user_id, user_name, user_message, False, character_data)
    # send_gen_request = await requests.post(f"{SERVER}/api/v1/chat", json=prompt) 
    async with aiohttp.ClientSession() as session:
        url = f"{SERVER}/api/v1/chat"
        send_gen_request = await fetch_post(session, url, prompt)
    if len(num_of_messages) % 15 == 0:
    # create a seperate thread for this 
        # handler.update_user_data_to_collection(str(user_id))
        threading.Thread(target=handler.update_user_data_to_collection, args=(str(user_id),)).start()
...
async def generate_response(user_id, user_name, user_message):
    ...
    async with aiohttp.ClientSession() as session:
            ...
            # Making the POST request non-blocking
            async with session.post(f"{SERVER}/api/v1/chat", json=prompt) as send_gen_request:
                if send_gen_request.status == 200:
                    result_json = await send_gen_request.json()
                    result = result_json['results'][0]['history']
                    response_text = remove_special(result['visible'][-1][1])
                    ...
                    if len(num_of_messages) % 15 == 0:
                        ...
                        loop.run_in_executor(None, handler.update_user_data_to_collection, str(user_id))
