...
async def generate_response(user_id, user_name, user_message):
    ...
    num_of_messages = await handler.read_user_file(str(user_id))
    if not len(num_of_messages) < 15:
        response_pairs = await handler.get_or_query_collection(str(user_id), user_message, n_results=3)
...
async def regenerate_message_llm(user_id, user):
    ...
    cache_history = await get_cache_history(user_id)
